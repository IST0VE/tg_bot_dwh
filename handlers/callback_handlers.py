...
def register_callback_handlers(bot: telebot.TeleBot):
    ...
    def handle_callback(call: CallbackQuery):
        ...
        if call.data == "up":
            ...
            # Найдём файл по file_id в текущей папке
            file = next((f for f in current["files"] if f.get("file_id") == file_id), None)
            if file:
                try:
                    if file["type"] == "text":
    ...
    def handle_shared_callback(call: CallbackQuery, data: dict):
        ...
        if command == "shared_up":
            ...
            # Найдём файл по file_id в текущей папке
            file = next((f for f in shared_folder["files"] if f.get("file_id") == file_id), None)
            if file:
                try:
                    if file["type"] == "text":
                        bot.send_message(call.message.chat.id, f"Текст: {file['content']}")
